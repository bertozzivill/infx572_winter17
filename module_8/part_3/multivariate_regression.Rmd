...
You'll notice something a little interesting about our new regression equation: It includes both *continuous* variables (like "years since Ph.D" and "years of service") and *categorical* variables like "sex" and "rank". These are treated very differently by the regression, and have different interpretations. Remeber this guideline: **continuous varaiables add slopes, categorical variables add intercepts**. I'll go over them below, using simplified versions of the full regression equation. 
...
It becomes impossible to visualize regressions this way when you add more than two continuous predictor variables, but conceptually the idea is exactly the same-- every time you add a continouous predictor, you're adding a dimension to your regression space, and adding a slope to your regression line. 
...
ggplot(Salaries, aes(x=yrs.since.phd)) +
  ...
  geom_line(aes(y=reg.cat.sex, color=sex), size=1) +
...
2. This dataoint is *not* a member of any other group in the list of options.
...
Whereas for men, the intercept value is uniformly higher:
