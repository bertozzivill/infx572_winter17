...
Salaries[, cont.reg.pred:=predict(cont_reg)]

p <- plot_ly(Salaries) %>%
  add_trace(x = ~yrs.since.phd, y = ~yrs.service, z = ~salary, name="data", type = 'scatter3d', mode='markers',  opacity=0.5, marker=list(size=3)) %>%
  add_trace(x= ~yrs.since.phd, y= ~ yrs.service, z=~cont.reg.pred, name="prediction", type= 'scatter3d', mode='lines', line=list(size=3))
