...
1. Aha! an `if`! There should be a logical statement coming up for me to evaluate!   
2. The logical statement evaluates to `TRUE`! I should run all the code inside the `if` statement's curly brackets and ignore the `else` statement completely!
...
### While
While loops are used less commonly, and are used when you want to iterate over something an unspecified number of times, until a specific condition is achieved. Here's an example:
```{r}
this_variable <- 1
while (this_variable/2<10){
